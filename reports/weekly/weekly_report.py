...
from reports.weekly.weekly_top_template import (
    weekly_top_template_from_excel,
...
def get_ga4_data_local(input_filename):
    """
    Load GA4 data from a local CSV file (with GA4 export format).
    """
    df = pd.read_csv(input_filename, header=9)
    df = drop_if_not_ends_with_html(df)
...
def get_ga4_data(source="local", **kwargs):
    ...
    """
    if source == "local":
        return get_ga4_data_local(kwargs["input_filename"])
    elif source == "api":
...
def run_weekly_report(
    ...
    df["Is_si_fara"] = df["pagePath"].apply(si_fara_func)
    # Map si farà articles
    df.loc[df["Is_si_fara"], "Categoria"] = "Si farà"
    ...
    for metric_col in ["activeUsers", "screenPageViews"]:
    ...
    df["Title"] = titles
    # Generate summaries
    # For each category, get top articles by views, saving results to an dictionary
    all_results = {}
    for category in df["Categoria"].unique():
        category_df = df[df["Categoria"] == category]
        top_articles = category_df.nlargest(n, sort_by_metric)
        all_results[category] = top_articles.to_dict(orient="records")
    # Save all results as a json
    all_results_json_path = os.path.join(output_dir, "top_articles_by_category.json")
    with open(all_results_json_path, "w") as f:
        json.dump(all_results, f)
    print(f"Top articles by category saved to {all_results_json_path}")
...
if __name__ == "__main__":
    ...
    weekly_output = run_weekly_report(
        ...
        excel_output_path=os.path.join(
            WEEKLY_OUTPUT_DIR, "top_articles_by_category.xlsx"
        ),
