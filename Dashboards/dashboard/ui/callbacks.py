...
def register_callbacks(
    app, analytics_service: AnalyticsService, trend_service: TrendService
):
    """
    ...
    def update_dashboard(n_clicks, start_date, end_date, granularity, comparison_type):
        ...
        try:
            ...
            metrics_cards = create_metrics_cards(
                summary_stats, comparison_metrics, trend_direction
            )

            # Generate insight
            insight_data = insight_generator.generate_trend_insight(
                comparison_metrics, trend_direction, (start, end), seasonality_info
            )
            ...
            monthly_views_chart = create_monthly_views_chart(df)
...
def create_metrics_cards(summary_stats, comparison_metrics, trend_direction):
    ...
    """
    trend_symbols = {"growth": "↑", "decline": "↓", "stable": "→"}
            ...
            html.Div(
                [
                    ComponentFactory.create_metric_card(
                        ...
                        trend_indicator=trend_symbols.get(
                            comparison_metrics["direction"]
                        ),
                        color="#ff9800",
                    )
                ],
...
def create_seasonality_chart(seasonality_info):
    ...
    # Create bar chart
    colors = [
        "#2196F3" if day != seasonality_info.get("strongest_day") else "#4caf50"
...
def create_monthly_views_chart(df):
    """
    Create monthly views bar chart

    Args:
        df: DataFrame with date and screenPageViews columns
    ...
    # Aggregate by month
    monthly_data = df.groupby("year_month")["screenPageViews"].sum().reset_index()
    monthly_data["year_month"] = monthly_data["year_month"].astype(str)

    # Format month labels (e.g., "2025-01" -> "Gen 2025")
    month_names_it = {
        "01": "Gen",
        "02": "Feb",
        "03": "Mar",
    ...
    def format_month_label(year_month_str):
        ...
        return f"{month_names_it.get(month, month)} {year}"

    monthly_data["month_label"] = monthly_data["year_month"].apply(format_month_label)
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                y=monthly_data["screenPageViews"],
                marker_color="#1976D2",
                text=[f"{v:,.0f}" for v in monthly_data["screenPageViews"]],
                textposition="outside",
                hovertemplate="<b>%{x}</b><br>Visualizzazioni: %{y:,.0f}<extra></extra>",
            )
        ]
    )
    ...
    fig.update_layout(
        title=dict(
            text="Visualizzazioni Totali per Mese", font=dict(size=18, color="#333")
        ),
        xaxis_title="Mese",
        yaxis_title="Visualizzazioni Totali",
        plot_bgcolor="white",
        paper_bgcolor="white",
        font=dict(family="Arial, sans-serif", size=12, color="#666"),
        showlegend=False,
